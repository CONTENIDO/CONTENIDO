    ...
    <form action="front_content.php" method="post" name="new_user_forum">
        ...
        <div style="padding-top:10px;">
            ...
            {if !empty($INPUT_FORUM_QUOTE)}
                ...
                <div>
                    <textarea class="input_forum_text" name="forum_quote"
                              tabindex="3">{$INPUT_FORUM_QUOTE|escape}</textarea>
                </div>
            {/if}

            <div class="yourcomment"><strong>{$FORUM|escape}</strong></div>
            <div>
                <textarea class="input_forum_text" name="forum"
                          tabindex="4">{$INPUT_FORUM|escape}</textarea>
            </div>
            {if isset($MODEMODETEXT)}
                <div class="modtext"><p>{$MODEMODETEXT|escape}</p></div>
            {/if}
            <div class="submitbuttons">
                <a href="{$CANCEL_LINK|escape}"
                   class='cancel button grey'>{$CANCEL_FORUM|escape}</a>
                <a href="javascript:document.forms['new_user_forum'].submit();"
                   class="submit button red">{$SAVE_FORUM|escape}</a>
            </div>
        </div>
    </form>
</div>
