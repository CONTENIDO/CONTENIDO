...
function writeTime(twitt_time) {
    ...
    var text = ''
    if (dt < 60) {
        text = translations_twitter ['less_minutes'];
    } else if (dt < 120) {
        text = translations_twitter ['one_minute'];
    } else if (dt < (45 * 60)) {
        text = translations_twitter ['more_minutes'].sprintf(Math.round(dt / 60));
    } else if (dt < (90 * 60)) {
        text = translations_twitter ['one_hours'];
    } else if (dt < (24 * 60 * 60)) {
        text = translations_twitter ['more_hours'].sprintf(Math.round(dt / 3600));
    } else if (dt < (48 * 60 * 60)) {
        text = translations_twitter ['one_day'];
    } else {
        text = translations_twitter ['more_days'].sprintf(Math.round(dt / 86400));
    }
...
function callTwitterSmall(tweeter_json) {
    ...
    for (var i = 0; i < anzahlObjekte; i++) {
        ...
        var timeObjekt = parseDate(tweeter_json[i].user.created_at);//new Date(Date.parse(tweeter_json[i].created_at));
