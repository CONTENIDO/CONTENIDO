 ...
 *
 *   $Id: editor.php 1157 2010-05-20 14:10:43Z xmurrix $:
 * }}
 * 
 */
...
//  ['strikethrough','subscript','superscript','separator'],
    ...
    ['custom1','custom2','custom3','anchor']
    ];
...
// modified set a classname which should be around the text
...
//modified commandID for my own Buttons
...
this.btnList = {
    ...
    "createlink":     ['link',                 'Insert Web Link',    'editor_action(this.id)',  'ed_link.gif'],
    ...
    "custom1":           ['custom1',         'Strip all HTML TAGS',  'editor_action(this.id)',  'ed_custom.gif'],
    "custom2":           ['custom2',         'Strip Format Tags',    'editor_action(this.id)',  'ed_custom.gif'],
    "custom3":           ['custom3',         'Select all',           'editor_action(this.id)',  'ed_custom.gif'],
    "anchor":            ['anchor',          'Anchor',               'editor_action(this.id)',  'ed_custom.gif'],
   // end: custom buttons
...
function editor_action(button_id) {
  ...
  // modified the action for this custom button
  // Custom1  cuts all HTML tags
  else if (cmdID == 'custom1') {

          sRange  = editdoc.selection.createRange();
          var jetzt   = sRange.htmlText;

          jetzt = jetzt.replace(/<script[^>]*?>.*?<\/script>/gi,"");
          jetzt = jetzt.replace(/<[\/\!]*?[^<>]*?>/gi,"");
          jetzt = jetzt.replace(/([\r\n])[\s]+/gi,"");
          jetzt = jetzt.replace(/&(quot|#34);/gi,"\"");
          jetzt = jetzt.replace(/&(amp|#38);/gi,"&");
          jetzt = jetzt.replace(/&(lt|#60);/gi,"<");
          jetzt = jetzt.replace(/&(gt|#62);/gi,">");
          jetzt = jetzt.replace(/&(nbsp|#160);/gi," ");


          sRange.pasteHTML('<p>'+jetzt+'</p>');
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
   ...
   // modified the action for this button
      // insert link
      ...
      // modified the action for this button and the file insert image.php
      // insert image
    ...
    // insert table
      ...
      showModalDialog(_editor_url + "popups/insert_table.html?"+objname,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; dialogWidth:430px; dialogHeight: 220px;");
    }


    //modified
    // insertanchor
    else if (cmdID.toLowerCase() == 'anchor'){

      sRange  = editdoc.selection.createRange();
      if (sRange.htmlText) {
          alert("Please dont select Content!")
      }else{
            var anchorname=prompt('Anchor Name:')
            if(anchorname!=''&&anchorname!='undefined'){
               editor_insertHTML(objname,'<a name="'+anchorname+'"></a>');
            }
      }
    ...
    // modified the action for this button
    // set tags for bold
    else if (cmdID.toLowerCase() == 'bold'){
         editor_insertHTML(objname,'<b>','</b>');
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
    ...
    html += '<style>\n';
    //modified   style for td
    html += 'td{border:1px dashed #cccccc}\n';
    ...
    //modified classname
    if(config.classname=='false'){
       html += '<body contenteditable="true" topmargin=1 leftmargin=1';
    }else{
       html += '<body contenteditable="true" topmargin=1 leftmargin=1 class='+config.classname;
    }
