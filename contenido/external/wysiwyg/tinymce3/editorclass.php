...
class cTinyMCEEditor extends cWYSIWYGEditor
{
    ...
    public function __construct($editorName, $editorContent)
    {
        ...
        $this->_aSettings['langid'] = $this->_lang;
        $this->_aSettings['clientid'] = $this->_client;
        ...
        // modified 23.10.2006
        $this->setSetting(null, 'article_url_suffix', 'front_content.php?idart=' . $this->_idart, true);

        // Default values
        $this->setSetting(null, 'mode', 'exact');
        $this->setSetting(null, 'content_css', $this->_sFrontendUrl . 'css/style_tiny.css');
        ...
        $this->setSetting(null, 'language', $langs[$this->_belang][4]);
        unset($langs);

        // Set document base URL
        $this->setSetting(null, 'document_base_url', $this->_sFrontendUrl, true);

        // The following 'base URL' is the URL used to reference JS script files
        // - it is not the base href value
        //$this->setBaseURL(preg_replace('/^https?:\/\/[^\/]+(.*)$/', '$1', $this->_getEditorUrl()));
        $this->setBaseURL($this->_getEditorUrl());
        ...
        $sDirection = langGetTextDirection($this->_lang);
        ...
        foreach (['contenido_toolbar_mode', 'contenido_lists'] as $key) {
            if (isset($this->_aSettings[$key])) {
                unset($this->_aSettings[$key]);
            }
        }

        // Remove obsolete, deprecated values
        foreach (['tinymce-stylesheet-file', 'tinymce-valid-elements', 'tinymce-extended-valid-elements', 'tinymce-lists', 'tinymce-styles', 'tinymce-toolbar-mode', 'tinymce-toolbar1', 'tinymce-toolbar2', 'tinymce-toolbar3', 'tinymce-plugins'] as $key) {
            if (isset($this->_aSettings[$key])) {
                unset($this->_aSettings[$key]);
            }
        }
    ...
    public function setLists($lists = '')
    {
        ...
        if (in_array('link', $aLists)) {
            $this->setSetting(null, 'external_link_list_url', $this->_baseURL . 'list.php?mode=link&lang=' . $this->_lang . '&client=' . $this->_client . '#', true);
        }
        if (in_array('image', $aLists)) {
            $this->setSetting(null, 'external_image_list_url', $this->_baseURL . 'list.php?mode=image&lang=' . $this->_lang . '&client=' . $this->_client . '#', true);
        }
        if (in_array('media', $aLists)) {
            $this->setSetting(null, 'media_external_list_url', $this->_baseURL . 'list.php?mode=media&lang=' . $this->_lang . '&client=' . $this->_client . '#', true);
        }
    }
    ...
    public function setToolbar($mode = '')
    {
        switch ($mode) {
            ...
            case 'inline_edit':
                ...
                $this->setSetting(null, 'content_css', $this->_sFrontendUrl . 'css/style_tiny.css', true);
    ...
    {
        $sessId = cRegistry::getBackendSessionId();
        ...
        foreach ($aParameters as $sParameter) {
            if (array_key_exists($sParameter, $this->_aSettings) && preg_match('/\\.php$/i', $this->_aSettings[$sParameter])) {
                $this->setSetting(null, $sParameter, $this->_aSettings[$sParameter] . '?contenido=' . $sessId, true);
            }
        }
    }
    ...
    public function getEditor()
    {
        ...
        $template->set('s', 'IMAGEBROWSER', $this->_sBackendUrl . 'frameset.php?area=upl&contenido=' . $sess->id . '&appendparameters=imagebrowser');
        $template->set('s', 'FILEBROWSER', $this->_sBackendUrl . 'frameset.php?area=upl&contenido=' . $sess->id . '&appendparameters=filebrowser');
        $template->set('s', 'MEDIABROWSER', $this->_sBackendUrl . 'frameset.php?area=upl&contenido=' . $sess->id . '&appendparameters=imagebrowser');
        $template->set('s', 'FRONTEND_PATH', $this->_sFrontendUrl);
        ...
        $return = $template->generate($this->_getEditorPath() . 'tinymce.tpl.html', true);
    ...
    public function getConfigInlineEdit()
    {
        ...
        return cString::getPartOfString($config, 0, -3);
    }
