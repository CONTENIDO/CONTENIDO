...
abstract class cContentTypeAbstractTabbed extends cContentTypeAbstract
{
    ...
    protected function _generateTabMenuCode(array $tabs)
    {
        ...
        $filename = $this->_cfg['path']['contenido'] . 'templates/standard/template.cms_abstract_tabbed_edit_tab_menu.html';
    ...
    {
        $template = new cTemplate();
        $filename = $this->_cfg['path']['contenido'] . 'templates/standard/template.cms_abstract_tabbed_edit_action.html';
        $code     = $template->generate($filename, true);
