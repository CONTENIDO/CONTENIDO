...
class ConUser extends ConUser_Abstract {
            ...
            if (!$this->oDb->query($sSql)) {
                ...
                if (!is_null($sNewPass)) {
                    ...
                    if ($iPassResult != iConUser::PASS_OK) {
                        ...
                        throw new ConUserException("Given password is not valid! [#" . $iPassResult . ']', iConUser::EXCEPTION_PASSWORD_INVALID);
                    }
                }
    ...
    public function userExists($sUserId) {
        ...
        $sSql = "SELECT COUNT(*) AS user_cnt
                 FROM `" . $this->aCfg["tab"]["phplib_auth_user_md5"] . "`
                 WHERE user_id = '" . Contenido_Security::escapeDB(strtolower($sUserId), $this->oDb) . "'";
    ...
    public function usernameExists($sUsername) {
        ...
        $sSql = "SELECT COUNT(*) AS user_cnt
                 FROM `" . $this->aCfg["tab"]["phplib_auth_user_md5"] . "`
                 WHERE LOWER(`username`) = '" . Contenido_Security::escapeDB(strtolower($sUsername), $this->oDb) . "'";
    ...
    public function checkPasswordStrength($sNewPassword) {
        ...
        if (!function_exists('crack_opendict')) {
            return $iResult;
        } elseif (!isset($this->aCfg['password']['use_cracklib']) || $this->aCfg['password']['use_cracklib'] != true) {
            return $iResult;
        } elseif (!isset($this->aCfg['password']['cracklib_dict']) || empty($this->aCfg['password']['cracklib_dict'])) {
            return $iResult;
        }
    ...
    public static function getErrorString($iErrorCode, $aCfg) {
        ...
        switch ($iErrorCode) {
            case iConUser::PASS_NOT_ENOUGH_MIXED_CHARS:
                $sError = sprintf(i18n("Please use at least %d lower and upper case characters in your password!"), $aCfg['password']['mixed_case_mandatory']);
                break;
            case iConUser::PASS_NOT_ENOUGH_NUMBERS:
                $sError = sprintf(i18n("Please use at least %d numbers in your password!"), $aCfg['password']['numbers_mandatory']);
                break;
            case iConUser::PASS_NOT_ENOUGH_SYMBOLS:
                $sError = sprintf(i18n("Please use at least %d symbols in your password!"), $aCfg['password']['symbols_mandatory']);
                break;
            case iConUser::PASS_TO_SHORT:
                $sError = sprintf(i18n("Password is too short! Please use at least %d signs."), ($aCfg['password']['min_length'] > 0 ? $aCfg['password']['min_length'] :
