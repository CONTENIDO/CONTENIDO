 ...
 * 
 * Description: 
 * Class for handling CMS Type Media
 * 
 * Requirements: 
 * @con_php_req 5.0
 * 
...
/**
 * This class provides all methods for the content type CMS_MEDIA. All properties of the file list are
...
class Cms_Media {
	...
	/**
	 * Current id of content type CMS_MEDIA[3] -> 3
	 * @var 	integer
	 * @access 	private
	 */
	private $iId = 0;
	 ...
	 * @param integer $iNumberOfCms - CMS_MEDIA[4] => 4
	...
	private function storeFileList() {	
		...
		conSaveContentEntry($this->iIdArtLang, 'CMS_MEDIA', $this->iId, $oXmlDom->saveXML(), true);
	...
	public function getAllWidgetEdit() {	
		...
		$oTpl->set('s', 'CONFIGS', 								i18n("Options"));
		...
		$sCode = $oTpl->generate($this->aCfg['path']['contenido'].'templates/standard/template.cms_media_edit.html', 1);
	...
	public function getAllWidgetView() {
		$sCode = '";?><?php
					$oFileList = new Cms_Media(\'%s\', %s, 0, "", $cfg, null, "", $client, $lang, $cfgClient, null);
