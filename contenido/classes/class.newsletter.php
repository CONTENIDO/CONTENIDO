...
* Modified  :   $Date: 2007/08/19 22:18:24 $
*
* © four for business AG, www.4fb.de, generated by HerrB (01.08.2004)
*
* $Id: class.newsletter.php,v 1.14 2007/08/19 22:18:24 bjoern.behrens Exp $
******************************************/
...
class NewsletterCollection extends ItemCollection
{
	...
	*/
	...
	/**
	* Overriden delete method to delete html newsletter article
	* @param integer $iItemID	The ID of the item that has to be deleted
	*/
	/* Disabled, as very slow - the html message article will be deleted in 
	 * include.newsletter_edit.php
	 * 
	function delete($iItemID) {
		cInclude ("includes", "functions.con.php"); // I'm not sure, if this is a good idea...
		
		$oItem = new Newsletter($iItemID);
		
		if ($oItem->get("type") == "html" && $oItem->get("idart") > 0) {
			conDeleteArt($oItem->get("idart"));
		}
		
		unset ($oItem);
		
		return parent::delete($iItemID);
	}
	*/
}
...
class Newsletter extends Item
{
	...
	*/
	...
	function _deChunkHTTPBody ($sHeader, $sBody, $sEOL = "\r\n")
	{
		...
		/* Old code, which had some problems ... remove on next release ...
		// Based on: http://www.html-world.de/program/phpex_7.php#socketverbindungen
		
		// Body chunked?
		if (strpos(strtolower($sHeader), 'transfer-encoding: chunked') !== false)
		{
			$sTrail = $sBody;
			while (strlen($sTrail) > 0)
			{
				// Remove first number
				$j = strpos($sBody, "\r\n");

				if (substr_count($sBody, "\r\n") > 0)
				{
					$sChar = substr($sBody, 0, $j);
					$sBody = substr($sBody, $j + 2);
					
					if (is_int(hexdec($sChar)))
					{
						$iChunkSize = hexdec($sChar);
						
						$sTrail = substr($sBody, $iChunkSize + 2);
						$sBody	= substr($sBody, 0, $iChunkSize);
						
						// Remove last number
						$j = strpos($sTrail, "\r\n");
						
						if (substr_count($sTrail, "\r\n") > 0)
						{
							$sChar = substr($sTrail, 0, $j);
							
							if ($sChar == '0') {
								break;
							}
							
							$sTrail = substr($sTrail, $j + 2);
						} else {
							break;
						}
					} else {
	...
	*/
	function send ($idcatart, $destination = "other", $to = "", $iChunkSize = 0, $iChunk = 0, $sEncoding = "iso-8859-1")
	{
		...
		global $cfg, $cfgClient, $client, $lang, $auth, $recipient;
		...
		if ($iChunkSize == 0) {
			...
			$sLimit = ($iChunk * $iChunkSize).", " .$iChunkSize;
		}
		...
		switch ($destination)
		{
			case "all" :
				// $oRecipients = new RecipientCollection;
				// $oRecipients->select("deactivated='0' AND confirmed='1' AND idclient='$client' AND idlang='$lang'", "", "", $sLimit);
				$sDistinct 	= "";
				$sFrom		= "";
				$sSQL 		= "deactivated='0' AND confirmed='1' AND idclient='$client' AND idlang='$lang'";
				break;
			case "default" :
				...
				/* $oRecipients = new RecipientGroupMemberCollection;
				
				$oRecipients->link('RecipientCollection');
				$oRecipients->link('RecipientGroupCollection');
				$oRecipients->setWhere('RecipientCollection.idclient',$client);
				$oRecipients->setWhere('RecipientCollection.idlang',$lang);
				$oRecipients->setWhere('RecipientCollection.deactivated','0');
				$oRecipients->setWhere('RecipientCollection.confirmed','1');
				$oRecipients->setWhere('RecipientGroupCollection.defaultgroup','1');
				$oRecipients->setWhere('RecipientGroupCollection.idclient',$client);
				$oRecipients->setWhere('RecipientGroupCollection.idlang',$lang);
				$oRecipients->query(); */

				// $oRecipients = new RecipientCollection;
				/* $oRecipients->flexSelect("distinct", $cfg["tab"]["news_groups"]." AS groups, ".
							$cfg["tab"]["news_groupmembers"]." AS groupmembers ", 
							"recipientcollection.idclient = '".$client."' AND ".
							"recipientcollection.idlang = '".$lang."' AND ".
							...
							"groups.defaultgroup = '1' AND groups.idclient = '".$client."' AND ".
							"groups.idlang = '".$lang."'", "", "", $sLimit); */
				$sDistinct 	= "distinct";
				$sFrom		= $cfg["tab"]["news_groups"]." AS groups, ".$cfg["tab"]["news_groupmembers"]." AS groupmembers ";
                                $sSQL		= "recipientcollection.idclient = '".$client."' AND ".
						  "recipientcollection.idlang = '".$lang."' AND ".
						  "recipientcollection.deactivated = '0' AND ".
						  "recipientcollection.confirmed = '1' AND ".
						  "recipientcollection.idnewsrcp = groupmembers.idnewsrcp AND ".
			...
			case "selection" :
				...
				if (is_array($to) && count($to) > 0)
				{
					...
					// $oRecipients->flexSelect("distinct", $cfg["tab"]["news_groupmembers"]." AS groupmembers ", "recipientcollection.idclient = '".$client."' AND "."recipientcollection.idlang = '".$lang."' AND "."recipientcollection.deactivated = '0' AND "."recipientcollection.confirmed = '1' AND "."recipientcollection.idnewsrcp = groupmembers.idnewsrcp AND "."groupmembers.idnewsgroup IN (".$groups.")", "", "", $sLimit);
					$sDistinct 	= "distinct";
					...
					$sSQL		= "recipientcollection.idclient = '".$client."' AND ".
							  ...
							  "recipientcollection.confirmed = '1' AND ".
							  "recipientcollection.idnewsrcp = groupmembers.idnewsrcp AND ".
							  "groupmembers.idnewsgroup IN (".$groups.")";
				} else {
					$destination = "other";
				}
				break;
			case "single" :
				if (is_numeric($to))
				{
					// $oRecipients->select("idnewsrcp = '".$to."'");
					$sDistinct 	= "";
					$sFrom		= "";
					$sSQL		= "idnewsrcp = '".$to."'";
				} else {
		...
		if ($destination == "other")
		{
			...
			$iTotalCount = $oRecipients->count();

			$oRecipients->flexSelect($sDistinct, $sFrom, $sSQL, "", "", $sLimit);
			$iBlockCount = $oRecipients->count();
			if ($iTotalCount == 0)
			{
				$result[] = 0;
				$result[] = i18n("No recipients found");
			} else if ($iBlockCount == 0) {
				$result[] = 0;
				...
				$message = str_replace("MAIL_NUMBER", $iTotalCount,           $message);
				$message = str_replace("MAIL_DATE",   strftime($sFormatDate), $message);
				$message = str_replace("MAIL_TIME",   strftime($sFormatTime), $message);
				...
				$sPath = $cfgClient[$client]["path"]["htmlpath"]."front_content.php?changelang=".$lang."&idcatart=".$idcatart."&";
					...
					if (is_array($cfg['plugins']['recipients']))
					{
						foreach ($cfg['plugins']['recipients'] as $plugin)
						{
							...
							if (function_exists("recipients_".$plugin."_wantedVariables")) {
								$aPlugins[$plugin] = call_user_func("recipients_".$plugin."_wantedVariables");
							}	
						}
					}
				} else {
				...
				// Don't change name of $recipient variable as it is used in plugins!
				while ($recipient = $oRecipients->next())
				{
