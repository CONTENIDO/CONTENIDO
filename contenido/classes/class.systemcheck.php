...
class cSystemCheck extends cSetupMask {
    ...
    public function __construct($step, $previous, $next, DB_Contenido $oDb, array $aCfg, $iClient, $iLang, array $aCfgClient) {
        ...
        $_SESSION["setuptype"] = $step;
        $_SESSION["dbhost"] = $aCfg['db']['connection']['host'];
        $_SESSION["dbuser"] = $aCfg['db']['connection']['user'];
        $_SESSION["dbname"] = $aCfg['db']['connection']['database'];
        $_SESSION["dbpass"] = $aCfg['db']['connection']['password'];

        $this->cSystemCheck($step, $previous, $next);
    }
    ...
    function cSystemCheck($step, $previous, $next) {
        cSetupMask::cSetupMask("templates/systemcheck/systemtest.tpl", $step);
        ...
        foreach ($this->_aMessages as $severity => $messageEntry) {
            switch ($severity) {
                case 2:
                    $icon = "../setup/images/icons/info.png";
                    $iconDescription = i18n("Information");
                    break;
                case 3:
                    $icon = "../setup/images/icons/warning.png";
                    $iconDescription = i18n("Warning");
                    break;
                case 4:
                    $icon = "../setup/images/icons/error.png";
        ...
        if (count($cHTMLFoldableErrorMessages) == 0) {
            $cHTMLFoldableErrorMessages[] = new cHTMLFoldableErrorMessage(i18n("No problems detected"), i18n("System integrity could not detect any problems with your system environment"), "../setup/images/icons/info.png");
        }
    ...
    function doPHPTests() {
        ...
        switch ($result) {
            ...
            case CON_BASEDIR_RESTRICTIONSUFFICIENT:
                $this->runTest(false, self::C_SEVERITY_INFO, i18n("open_basedir setting might be insufficient"), i18n("System integrity believes that the PHP directive open_basedir is configured sufficient, however, if you encounter errors like 'open_basedir restriction in effect. File <filename> is not within the allowed path(s): <path>', you have to adjust the open_basedir directive"));
                break;
            case CON_BASEDIR_INCOMPATIBLE:
                $this->runTest(false, self::C_SEVERITY_ERROR, i18n("open_basedir directive incompatible"), i18n("System integrity has checked your PHP open_basedir directive and reckons that it is not sufficient. Please change the directive to include the CONTENIDO installation or turn it off completely."));
                break;
        }
        ...
        switch ($result) {
            case CON_IMAGERESIZE_CANTCHECK:
                $this->runTest(false, self::C_SEVERITY_WARNING, i18n("Unable to check for a suitable image resizer"), i18n("System integrity has tried to check for a suitable image resizer (which is, for exampl, required for thumbnail creation), but was not able to clearly identify one. If thumbnails won't work, make sure you've got either the GD-extension or ImageMagick available."));
                break;
            case CON_IMAGERESIZE_NOTHINGAVAILABLE:
                $this->runTest(false, self::C_SEVERITY_ERROR, i18n("No suitable image resizer available"), i18n("System integrity checked your image resizing support, however, it was unable to find a suitable image resizer. Thumbnails won't work correctly or won't be looking good. Install the GD-Extension or ImageMagick"));
                break;
        }
    ...
    function doMySQLTests() {
        ...
        $this->runTest($status, self::C_SEVERITY_ERROR, i18n("MySQL database connect failed"), sprintf(i18n("System integrity was unable to connect to the MySQL Server (Server %s, Username %s). Please correct the MySQL data and try again.<br><br>The error message given was: %s"), $_SESSION["dbhost"], $_SESSION["dbuser"], $errorMessage));
        ...
        switch ($_SESSION["setuptype"]) {
            case "setup":
                ...
                if ($status) {
                    ...
                    if ($db->next_record()) {
                        $this->runTest(false, self::C_SEVERITY_ERROR, i18n("MySQL test table already exists in the database"), sprintf(i18n("System integrity checked the database %s and found the test table %s. Please remove it before continuing."), $_SESSION["dbname"], sprintf("%s_test", $_SESSION["dbprefix"])));
                        return;
                    }
                    ...
                    if (!$status) {
                        $this->runTest(false, self::C_SEVERITY_ERROR, i18n("Unable to create tables in the selected MySQL database"), sprintf(i18n("System integrity tried to create a test table in the database %s and failed. Please assign table creation permissions to the database user you entered, or ask an administrator to do so."), $_SESSION["dbname"]));
                        return;
                    }
                    ...
                    if (!$status) {
                        $this->runTest(false, self::C_SEVERITY_WARNING, i18n("Unable to remove the test table"), sprintf(i18n("System integrity tried to remove the test table %s in the database %s and failed due to insufficient permissions. Please remove the table %s manually."), sprintf("%s_test", $_SESSION["dbprefix"]), $_SESSION["dbname"], sprintf("%s_test", $_SESSION["dbprefix"])));
                    }
                } else {
                    $db->connect();
                    ...
                    if (!$status) {
                        $this->runTest(false, self::C_SEVERITY_ERROR, i18n("Unable to create the database in the MySQL server"), sprintf(i18n("System integrity tried to create a test database and failed. Please assign database creation permissions to the database user you entered, ask an administrator to do so, or create the database manually.")));
                        return;
                    }
            ...
            case "upgrade":
                ...
                $status = checkMySQLDatabaseExists($db, $_SESSION["dbname"]);
                if (!$status) {
                    $this->runTest(false, self::C_SEVERITY_ERROR, i18n("No data found for the upgrade"), sprintf(i18n("System integrity tried to locate the data for the upgrade, however, the database %s doesn't exist. You need to copy your database first before running system integrity."), $_SESSION["dbname"]));
                    return;
                }
                ...
                $db->query(sprintf($sql, $_SESSION["dbprefix"]));
                if (!$db->next_record()) {
                    $this->runTest(false, self::C_SEVERITY_ERROR, i18n("No data found for the upgrade"), sprintf(i18n("System integrity tried to locate the data for the upgrade, however, the database %s contains no tables. You need to copy your database first before running system integrity."), $_SESSION["dbname"]));
                    return;
                }
    ...
    function logFilePrediction($file, $severity, $filetype) {
        ...
        $message = sprintf(i18n("System integrity or CONTENIDO can't write to the file %s. Please change the file permissions to correct this problem."), $shortFilename);
            ...
            if (cFileHandler::exists($file)) {
                ...
                switch ($perm) {
                    case CON_PREDICT_WINDOWS:
                        $predictMessage = i18n("Your Server runs Windows. Due to that, System integrity can't recommend any file permissions.");
                        break;
                ...
                switch ($perm) {
                    case CON_PREDICT_WINDOWS:
                        $predictMessage = i18n("Your Server runs Windows. Due to that, System integrity can't recommend any directory permissions.");
                        break;
                    case CON_PREDICT_NOTPREDICTABLE:
