 ...
 * @subpackage ModRewrite
 */
class PiModRewriteTestController extends PiModRewriteControllerAbstract
{
    ...
    public function testAction()
    {
        ...
        // Instance of mr test
        $oMRTest = new PiModRewriteTestService($this->maxItems);

        $startTime = getmicrotime();

        // Fetch complete CONTENIDO page structure
        $catArtStruct = $oMRTest->fetchFullStructure();
        PiModRewriteDebugger::add($catArtStruct, __METHOD__ . ' $catArtStruct');
        ...
        $mrUrlStack = PiModRewriteUrlStackService::getInstance();
        ...
        foreach ($test as $v) {
            $url = PiModRewriteUtil::buildNewUrl($v['url']);

            $resolvedUrlDto = $oMRTest->resolveUrl($url);
            $error = '';
            ...
            // @todo: translate
            if ($resolvedUrlDto->getError() !== null) {
                switch ($resolvedUrlDto) {
                    case PiModRewriteFrontContentService::ERROR_CLIENT:
                        $error = 'client';
                        break;
                    case PiModRewriteFrontContentService::ERROR_LANGUAGE:
                        $error = 'language';
                        break;
                    case PiModRewriteFrontContentService::ERROR_CATEGORY:
                        $error = 'category';
                        break;
                    case PiModRewriteFrontContentService::ERROR_ARTICLE:
                        $error = 'article';
                        break;
                    case PiModRewriteFrontContentService::ERROR_POST_VALIDATION:
                        $error = 'validation';
                        break;
                }
            }
            ...
            $itemTpl = str_replace('{data}', $oMRTest->getReadableResolvedData($resolvedUrlDto->toArray()), $itemTpl);
...
/**
 * @deprecated Since Advanced Mod Rewrite 2.1.0, use {@see PiModRewriteTestController} instead
 */
class ModRewrite_ContentTestController extends PiModRewriteTestController
{}
