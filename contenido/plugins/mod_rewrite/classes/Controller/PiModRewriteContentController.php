 ...
 * @subpackage ModRewrite
 */
class PiModRewriteContentController extends PiModRewriteControllerAbstract
{
    ...
    public function saveAction()
    {
        ...
        PiModRewriteRequestUtil::cleanup($request);

        // use mod_rewrite
        if (PiModRewriteUtil::arrayValue($request, 'use') == 1) {
        ...
        // root dir
        if (PiModRewriteUtil::arrayValue($request, 'rootdir', '') !== '') {
            if (!preg_match('/^[a-zA-Z0-9\-_\/\.]*$/', $request['rootdir'])) {
                ...
                if (PiModRewriteUtil::arrayValue($request, 'checkrootdir') == 1) {
        ...
        // start from root
        if (PiModRewriteUtil::arrayValue($request, 'startfromroot') == 1) {
        ...
        if (PiModRewriteUtil::arrayValue($request, 'prevent_duplicated_content') == 1) {
        ...
        // language settings
        if (PiModRewriteUtil::arrayValue($request, 'use_language') == 1) {
            $this->view->use_language_chk = ' checked="checked"';
            $this->view->use_language_name_disabled = '';
            $mrCfg['mod_rewrite']['use_language'] = 1;
            if (PiModRewriteUtil::arrayValue($request, 'use_language_name') == 1) {
        ...
        // client settings
        if (PiModRewriteUtil::arrayValue($request, 'use_client') == 1) {
            $this->view->use_client_chk = ' checked="checked"';
            $this->view->use_client_name_disabled = '';
            $mrCfg['mod_rewrite']['use_client'] = 1;
            if (PiModRewriteUtil::arrayValue($request, 'use_client_name') == 1) {
        ...
        // use lowercase uri
        if (PiModRewriteUtil::arrayValue($request, 'use_lowercase_uri') == 1) {
        ...
        $categorySeperator = PiModRewriteUtil::arrayValue($request, 'category_seperator', '');
        $categoryWordSeperator = PiModRewriteUtil::arrayValue($request, 'category_word_seperator', '');
        $articleSeperator = PiModRewriteUtil::arrayValue($request, 'article_seperator', '');
        $articleWordSeperator = PiModRewriteUtil::arrayValue($request, 'article_word_seperator', '');
        ...
        // file extension
        if (PiModRewriteUtil::arrayValue($request, 'file_extension', '') !== '') {
        ...
        if (isset($request['category_resolve_min_percentage'])) {
            ...
            $this->view->category_resolve_min_percentage = cSecurity::toInteger($request['category_resolve_min_percentage']);
            $mrCfg['mod_rewrite']['category_resolve_min_percentage'] = cSecurity::toInteger($request['category_resolve_min_percentage']);
        } else {
        ...
        if (PiModRewriteUtil::arrayValue($request, 'add_startart_name_to_url') == 1) {
            $this->view->add_startart_name_to_url_chk = ' checked="checked"';
            $mrCfg['mod_rewrite']['add_startart_name_to_url'] = 1;
            if (PiModRewriteUtil::arrayValue($request, 'add_startart_name_to_url', '') !== '') {
        ...
        // rewrite urls at
        if (PiModRewriteUtil::arrayValue($request, 'rewrite_urls_at') == 'congeneratecode') {
        ...
        if (isset($request['rewrite_routing'])) {
            $aRouting = [];
            $routingItems = explode("\n", $request['rewrite_routing']);
            foreach ($routingItems as $routingItem) {
                $routingDef = explode($routingSeparator, $routingItem);
        ...
        if ($this->mrConfigurationService->setConfiguration($this->clientId, $mrCfg)) {
            if ($bSeparatorModified) {
                $this->mrConfigurationService->loadConfiguration($this->clientId, true);
            }
            $this->view->content_before = $this->renderNotification(
                'info',
            ...
            $this->view->content_before = $this->renderNotification(
                ...
                sprintf(
                    ...
                    $this->mrConfigurationService->getConfigurationFilePath($this->clientId)
                )
            );
        }
    }
...
/**
 * @deprecated Since Advanced Mod Rewrite 2.1.0, use {@see PiModRewriteContentController} instead
 */
class ModRewrite_ContentController extends PiModRewriteContentController
{}
