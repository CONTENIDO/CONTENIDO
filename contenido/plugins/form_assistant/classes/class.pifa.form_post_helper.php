...
abstract class PifaAbstractFormProcessor {
    ...
    public function __construct($idform) {
        ...
        if (0 === $idform) {
            throw new ModuleException('don\'t know which form to load');
        }

        // load form
        $this->_form = new PifaForm($idform);
        if (false === $this->_form->isLoaded()) {
            throw new ModuleException('could not load form');
        }
    ...
    /**
     * Template method to postprocess data that has just been stored to database.
    ...
    public function process() {
        ...
        if (NULL === $this->_form) {
            throw new ModuleException('there is no form to process');
        }
 ...
 * @author marcus.gnass
 */
class PifaDefaultFormProcessor extends PifaAbstractFormProcessor {
