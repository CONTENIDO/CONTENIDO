 ...
 * 
 * Description: 
 * right_bottom frame for Tagging
 * 
 * Requirements: 
 * @con_php_req 5.0
 * 
 ...
 *
 *   $Id: include.right_bottom.php 2101 2012-04-03 12:46:11Z mischa.holz $:
 * }}
 * 
 */
...
$oPage->setMargin(10);
$oPage->setMessageBox();
$oTree = new pApiTaggingTreeView('f7771624-4874-4745-8b7e-21a49a71a447');

// store item
if ($_POST['step'] == 'store') { 
	$pNotify = '<div style="width:410px;margin-bottom:20px;">';
	$sMessage = sprintf(i18n("New Category %s successfully stored!", 'tagging'), $treeItem['name']);
...
if ($_POST['step'] == 'storeRename') { 
	$pNotify = '<div style="width:410px;margin-bottom:20px;">';
	$sMessage = sprintf(i18n("Category %s successfully renamed!", 'tagging'), $treeItem['name']);
    $notification->displayNotification("info", $sMessage);
	$pNotify .= '</div>';
	$oTree->storeItem($aPostTreeItem);
}
...
if ($_GET['step'] == 'deleteItem') { // delete item
	$pNotify = '<div style="width:410px;margin-bottom:20px;">';
	$sMessage = i18n("Category successfully deleted!", 'tagging');
    $notification->displayNotification("info", $sMessage);
	$pNotify .= '</div>';
...
if ($_GET['step'] == 'createRoot') { // create new root item
	$form = '
		...
		<tr><td colspan="2" class="text_medium">'.i18n("Create new tree", 'tagging').'</td></tr>
		<tr>
    ...
    $newTree = '<a href="main.php?action='.$action.'&step=createRoot&frame='.$frame.'&area='.$area.'&contenido='.$sess->id.'"><img  src="images/folder_new.gif" border="0" style="vertical-align: middle; margin-right: 5px;">'.i18n("Create new tree", 'tagging').'</a><div style="height:10px"></div>';
}
