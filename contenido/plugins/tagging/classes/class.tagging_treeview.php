 ...
 * 
 * Description: 
 * Stores and handles tagging management
 * 
 * Requirements: 
 * @con_php_req 5.0
 * 
 ...
 *
 *   $Id: class.tagging_treeview.php 2101 2012-04-03 12:46:11Z mischa.holz $:
 * }}
 * 
 */
 ...
 * @subpackage Tagging
 */
class pApiTaggingTreeView extends pApiTree {
	 ...
	 * 
	 */
	function pApiTaggingTreeView ($uuid) {
		global $cfg;
	...
	function _buildRenderTree ($tree) {
		...
		foreach ($tree as $item_tmp) {
			...
			if ($_GET['step'] == 'rename' && $item_tmp['idpica_alloc'] == $_GET['idpica_alloc']) {
				...
				$item['ITEMNAME'] = '
					...
					<script language="JavaScript">
						...
						function fieldCheck() {
							if (controller.value == "") {
								alert("'.i18n("Please enter a category name", 'tagging').'");
								controller.focus();
								return false;
							}
						return true;
			...
			$item['ACTION_CREATE'] = '<a href="main.php?contenido='.$sess->id.'&action='.$action.'&frame='.$frame.'&area='.$area.'&step=add&parentid='.$item_tmp['idpica_alloc'].'"><img src="images/folder_new.gif" border="0" title="'.i18n("New category", 'tagging').'" alt="'.i18n("New category", 'tagging').'" /></a>';
			
			$item['ACTION_RENAME'] = '<a href="main.php?contenido='.$sess->id.'&action='.$action.'&frame='.$frame.'&area='.$area.'&step=rename&idpica_alloc='.$item_tmp['idpica_alloc'].'"><img src="images/but_todo.gif" width="16" height="16" border="0" alt="'.i18n("Rename category", 'tagging').'" title="'.i18n("Rename category", 'tagging').'" /></a>';
			$item['ACTION_MOVE_UP'] = (count($result) >= 1) ? '<a href="main.php?contenido='.$sess->id.'&action='.$action.'&frame='.$frame.'&area='.$area.'&step=moveup&idpica_alloc='.$item_tmp['idpica_alloc'].'"><img src="images/folder_moveup.gif" border="0" alt="'.i18n("Move category up", 'tagging').'" title="'.i18n("Move category up", 'tagging').'" /></a>' : '<img src="images/spacer.gif" width="16" height="16" /></a>';
			$item['ACTION_MOVE_DOWN'] = (count($result) >= 1) ? '<img src="images/folder_movedown.gif" border="0" alt="'.i18n("Move category down", 'tagging').'" title="'.i18n("Move category down", 'tagging').'" />' : '<img src="images/spacer.gif" width="16" height="16" />';
			$item['ACTION_MOVE_DOWN'] = '';
			
			if ($item_tmp['online'] == 1) { // set offline
				$item['ACTION_ONOFFLINE'] = '<a href="main.php?contenido='.$sess->id.'&action='.$action.'&frame='.$frame.'&area='.$area.'&step=offline&idpica_alloc='.$item_tmp['idpica_alloc'].'""><img src="images/online.gif" alt="'.i18n("Set category offline", 'tagging').'" title="'.i18n("Set category offline", 'tagging').'"></a>';
			} else {
				$item['ACTION_ONOFFLINE'] = '<a href="main.php?contenido='.$sess->id.'&action='.$action.'&frame='.$frame.'&area='.$area.'&step=online&idpica_alloc='.$item_tmp['idpica_alloc'].'""><img src="images/offline.gif" alt="'.i18n("Set category online", 'tagging').'" title="'.i18n("Set category online", 'tagging').'"></a>';
			}
			
			if ($item_tmp['children']) {
				$item['ACTION_DELETE'] = '<img src="images/delete_inact.gif" border="0" alt="'.i18n("One or more subcategories exist, unable to delete", 'tagging').'" title="'.i18n("One or more subcategories exist, unable to delete", 'tagging').'">';
			} else {
				$name = str_replace("\"", "&amp;quot;", str_replace("'", "\'", $item_tmp['name']));
				$item['ACTION_DELETE'] = '<a href="javascript://" onclick="box.confirm(&quot;'.i18n("Delete category", 'tagging').'&quot;, &quot;'.i18n("Are you sure to delete the following category", 'tagging').':<br><br><b>'.$name.'</b>&quot;,&quot;deleteCategory('.$item_tmp['idpica_alloc'].')&quot;);"><img src="images/delete.gif" border="0" alt="'.i18n("Delete category").'" title="'.i18n("Delete category", 'tagging').'"></a>';
			}
	...
	function renderTree ($return = true) {
		...
		$this->tpl->set('s', 'CATEGORY', i18n("Category", 'tagging'));
		$this->tpl->set('s', 'ACTIONS', i18n("Actions", 'tagging'));
