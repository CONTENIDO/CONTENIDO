 ...
 * 
 * Description: 
 * Functions for tplcfg, use in combination with nclude.tplcfg_edit_form_config.php
 * Copy of the include.tplcfg_edit.php need for save the config in output mode
 * 
 * 
 * Requirements: 
 * @con_php_req 5.0
 ...
 *   modified 2011-08.10, Rusmir Jusufovic, save only once configuration and dont erase other
 *
 * }}
 * 
 */
...
if ( $idtpl != 0 && $idtplcfg != 0 ) {
        $sql = "SELECT number FROM ".$cfg["tab"]["container"]." WHERE idtpl = '".Contenido_Security::toInteger($idtpl)."' AND idmod='".Contenido_Security::toInteger($idmod)."'";
        $db->query($sql);
        $saveNumber = '';
        while ($db->next_record()) {

                $saveNumber= $db->f("number");
                $CiCMS_VAR = "C".$saveNumber."CMS_VAR";
                ...
                if (isset($tmp)) {
                        ...
                        foreach ($tmp as $key=>$value) {
                                ...
                                if (!isset($varstring[$saveNumber])) $varstring[$saveNumber]="";
                                $varstring[$saveNumber] = $varstring[$saveNumber].$key."=".$value."&";
                        }
        ...
        if (isset($varstring) && is_array($varstring)) {
            ...
            $sql = "DELETE FROM ".$cfg["tab"]["container_conf"]." WHERE idtplcfg = '".Contenido_Security::toInteger($idtplcfg)."' AND number='".$saveNumber."'";
