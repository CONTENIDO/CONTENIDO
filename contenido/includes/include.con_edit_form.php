...
if ($action == "con_newart" && $newart != true)
{
	...
	$perm->have_perm_area_action_item($area,"con_edit", $idcat)) {
		...
		$perm->have_perm_area_action_item("con","con_makeonline", $idcat)) && $inUse == false)
		{
			...
			$tpl->set('s', 'CHOOSEEND', '<img src="images/calendar.gif" width="16" height="16" style="vertical-align:top;margin-top:2px;" id="trigger_end" alt="'.i18n("Select end time").'">'.$sCalEndInit);
			$tpl->set('s', 'CHOOSESTART', '<img src="images/calendar.gif" width="16" height="16" style="vertical-align:top;margin-top:2px;" id="trigger_start" alt="'.i18n("Select start time").'">'.$sCalStartInit);
		} else {
