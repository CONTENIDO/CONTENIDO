...
$doclink  = "<select name=\"selectpdf\" size='1'>";
$doclink .= "<option value=\"\" selected>" . i18n("Please choose") . "</option>";

$filetypes = $cfg['content_types']['CMS_LINKEDIT']['document_filetypes'];
foreach ($filetypes as $p => $type) {
    $filetypes[$p] = $db->escape($type);
}
$filetypes = "'" . implode("','", $filetypes) . "'";

$sql = "SELECT * FROM " . $cfg["tab"]["upl"] . " WHERE idclient='" . cSecurity::toInteger($client) . "' AND filetype IN (" . $filetypes . ") ORDER BY dirname, filename";
...
$form->add(i18n("Link to a document"), $doclink);

// Image files links
$imglink  = "<select name=\"selectimg\" size='1'>";
$imglink .= "<option value=\"\" selected>" . i18n("Please choose") . "</option>";

$filetypes = $cfg['content_types']['CMS_LINKEDIT']['image_filetypes'];
foreach ($filetypes as $p => $type) {
    $filetypes[$p] = $db->escape($type);
}
$filetypes = "'" . implode("','", $filetypes) . "'";
...
// Archive files links
$archivelink  = "<select name=\"selectzip\" size='1'>";
$archivelink .= "<option value=\"\" selected>" . i18n("Please choose") . "</option>";

$filetypes = $cfg['content_types']['CMS_LINKEDIT']['archive_filetypes'];
foreach ($filetypes as $p => $type) {
    $filetypes[$p] = $db->escape($type);
}
$filetypes = "'" . implode("','", $filetypes) . "'";
...
$form->add(i18n("Link to an archive"), $archivelink);

// Media files links
$medialink  = "<select name=\"selectaudio\" size='1'>";
$medialink .= "<option value=\"\" selected>" . i18n("Please choose") . "</option>";

$filetypes = $cfg['content_types']['CMS_LINKEDIT']['media_filetypes'];
foreach ($filetypes as $p => $type) {
    $filetypes[$p] = $db->escape($type);
}
$filetypes = "'" . implode("','", $filetypes) . "'";
...
$form->add(i18n("Link to a media file"), $medialink);
