...
if ($action == "mod_delete") {
    ...
    if ($contenidoModuleHandler->eraseModule() == true) {
        ...
        $notification = $notificationClass->returnNotification(cGuiNotification::LEVEL_INFO, i18n("Deleted module successfully!"));
        $div = new cHTMLDiv($notification);
...
if (!$perm->have_perm_area_action_item("mod_edit", "mod_edit", $idmod)) {
    ...
    $name = new cHTMLTextbox("name", stripslashes_deep(conHtmlSpecialChars($module->get("name"))), 60);
    $descr = new cHTMLTextarea("descr", stripslashes_deep(conHtmlSpecialChars($module->get("description"))), 100, 5);
