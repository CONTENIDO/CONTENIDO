...
if ($oNewsletter->virgin == false && $oNewsletter->get("idclient") == $client && $oNewsletter->get("idlang") == $lang) {
	...
	{ 
		...
		$sMessage = stripslashes($_REQUEST["txtMessage"]);
	...
	/* $pluginOrder = trim_array(explode(",", getSystemProperty("plugin", "newsletters-pluginorder"))); */
	...
	/* if (is_array($pluginOrder))
	{
		...
		{
			if (function_exists("newsletters_".$plugin."_getTitle") && function_exists("newsletters_".$plugin."_display"))
			{
				$plugTitle = call_user_func("newsletters_".$plugin."_getTitle");
				$display = call_user_func("newsletters_".$plugin."_display", $oNewsletter);
	 			
				if (is_array($plugTitle) && is_array($display))
				{
		 			foreach ($plugTitle as $key => $value)
		 			{
		 				$oForm->add($value, $display[$key]);	
	 				}
		 		} else {
	 				if (is_array($plugTitle) || is_array($display))
	 				{
		 				$oForm->add(i18n("WARNING"), sprintf(i18n("The plugin %s delivered an array for the displayed titles, but did not return an array for the contents."), $plugin));
	 				} else {
	 					$oForm->add($plugTitle, $display);
	 				}
				}
			}
		}
