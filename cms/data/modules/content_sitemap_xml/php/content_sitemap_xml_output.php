<?php
/**
 * This module creates a XML sitemap.
 *
 * @package Module
 * @subpackage Sitemap XML
 *
...
function saveSitemap(SimpleXMLElement $sitemap, $filename = '') {
    if (empty($filename)) {
        ...
        $cfgClient = cRegistry::getClientConfig();
        $client = cRegistry::getClientId();
        $shortFilename = $cfgClient[$client]['xml']['frontendpath'] . $filename;
        $success = $sitemap->asXML($filename);
        if ($success) {
            $transString = mi18n("XML_SITEMAP_SUCCESS_WRITTEN", $filename);
        } else {
