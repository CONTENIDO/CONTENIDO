 ...
 * Selected, UserSelected) JoinMultiple: If JoinSel = UserSelected then: If more
 * than one group has been specified, select more than one JoinGroups: Selected
 * group(s) JoinMessageType: Message type for new recipients: User select
 * (user), text or html OptNewWindow: Open handler window in new browser window?
 */
$aSettings = array(
...
if ($aSettings["JoinSel"] == "UserSelected") {
    ...
    // defaultgroup = '0' AND idnewsgroup IN (".$aSettings["JoinGroups"].")","",
    // "groupname ASC");
