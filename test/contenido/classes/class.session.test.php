...
class cSessionTest extends cTestingTestCase {
    ...
    public function testUrlWidthParameter() {
        ...
        $expectedUrl = self::URL_BASE . '?' . http_build_query([$this->_session->name => $this->_session->id, 'foo' => 'bar']);
        $sessUrl = $this->_session->url($url);
        $this->assertEquals($expectedUrl, $sessUrl);
    }
    ...
    public function testUrlWithFragment() {
        $url = self::URL_BASE . '?' . http_build_query([$this->_session->name => $this->_session->id, 'foo' => 'bar']) . '#baz';

        $expectedUrl = self::URL_BASE . '?' . http_build_query([$this->_session->name => $this->_session->id, 'foo' => 'bar']) . '#baz';
        $sessUrl = $this->_session->url($url);
        $this->assertEquals($expectedUrl, $sessUrl);
    }
